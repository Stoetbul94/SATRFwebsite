...
export default async function handler(
  ...
  try {
    ...
    // Link scores to users via userId (Firebase UID)
    // Initialize Firebase Admin SDK to lookup users
    ...
    const scoresWithUserId: Array<ScoreData & { userId?: string }> = [];
    const userLookupCache: Map<string, string> = new Map(); // Cache name+club -> userId

    // Helper function to find user by name and club
    const findUserId = async (shooterName: string, club: string): Promise<string | undefined> => {
      const cacheKey = `${shooterName.toLowerCase()}|${club.toLowerCase()}`;
      if (userLookupCache.has(cacheKey)) {
        return userLookupCache.get(cacheKey);
      }

      try {
        // Try to find user by matching firstName + lastName and club
        const nameParts = shooterName.trim().split(/\s+/);
        if (nameParts.length >= 2) {
          const firstName = nameParts[0];
          const lastName = nameParts.slice(1).join(' ');

          // Query users by firstName, lastName, and club
          ...
          if (!usersSnapshot.empty) {
            ...
            userLookupCache.set(cacheKey, userId);
            return userId;
          }
        }

        // Fallback: try partial match on lastName and club
        ...
        console.error(`Error finding user for ${shooterName} (${club}):`, error);
      }

      return undefined;
    };

    // Link each score to a user
    for (const score of validScores) {
      const userId = await findUserId(score.shooterName, score.club);
      scoresWithUserId.push({
        ...score,
        userId, // Add userId if found, undefined if not found
      });
    }
    ...
    try {
      const backendResponse = await fetch(`${API_BASE_URL}/${API_VERSION}/admin/scores/import`, {
        ...
        body: JSON.stringify({ scores: scoresWithUserId }),
