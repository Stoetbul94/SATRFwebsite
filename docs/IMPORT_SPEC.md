...
This document is the contract between **`SATRF_Score_Import.xlsx`** (the per-match upload file, generated by `scripts/generate-score-import-template.js`) and the website importer (`src/lib/excelImport.ts`). It defines every sheet the importer reads, the exact columns, their types, and how each row maps to a `ScoreInput`. Keep this file and the template in sync — if one changes, change both.

A copy is also kept at `docs/templates/SATRF_Match_Template.xlsx` for reference.
...
- All scoring is decimal; integer columns are optional ring totals (recommended for 3-Position and F-Class qualification).
- `Veteran Y/N`: `Y` sets `isVeteran: true` while `category` stays `open` (Option B veteran scoring).
- **Re-import / re-score:** uploading the same shooter + event + discipline + stage again **soft-deletes** the prior score and replaces it (no duplicate rows).
...
| `F-Class` | from **Discipline** column (`fclass_open` / `fclass_tr`) | `qualification` | 6 series decimal (+opt integer ring counts) |
| `3-Position 50m` | `three_position_50m` | `qualification` | 3 position totals (decimal + optional ring int each) |
