...
function checkEnvironment() {
  ...
  if (fs.existsSync(testFilesDir)) {
    ...
    logWarning('Test files directory not found, will generate');
  }
...
function runE2ETests() {
  ...
  const testFilesDir = path.join(process.cwd(), 'test-files');
  if (!fs.existsSync(testFilesDir)) {
    logInfo('Generating test files...');
    const generateResult = runCommand('npm run test:generate-data', 'Test data generation');
    if (!generateResult.success) {
      ...
      testResults.details.push({
        category: 'E2E Tests',
        status: 'SKIPPED',
        output: 'Test files could not be generated'
      });
      return;
    }
  }
...
function generateReport() {
  ...
  testResults.details.forEach((detail, index) => {
    const statusColor = detail.status === 'PASSED' ? 'green' : 
                       detail.status === 'WARNING' ? 'yellow' : 
                       detail.status === 'SKIPPED' ? 'yellow' : 'red';
  ...
  const isReady = testResults.failed === 0 && parseFloat(passRate) >= 80;
  ...
  const reportData = {
    ...
    recommendations: {
      ...
      important: parseFloat(passRate) < 80 ? ['Improve test coverage'] : [],
...
function main() {
  logHeader('SATRF Website Launch Readiness Testing (Simplified)');
...
if (args.includes('--help') || args.includes('-h')) {
  log(`
Usage: node scripts/run-launch-tests-simple.js [options]
...
Examples:
  node scripts/run-launch-tests-simple.js
  node scripts/run-launch-tests-simple.js --quick
  node scripts/run-launch-tests-simple.js --verbose
`, 'cyan');
  process.exit(0);
}
